...
namespace DesignPatterns.LLD.AmazonLockerSystem {
    internal class MainTest {
        ...
        //3. Product listing is also there so that when a new product comes, he can list it
        //and decide what sort of storage it can be put in.
        ...
        //Add //5. When the QR is scanned, the appropriate locker is opened we can put it there.

        //6. Each locker is at a certain distance from the system, so the system takes care of that as well.

        //Remove//7. We can empty the locker as well.

        //Get //8. If we want a certain product, we can check in the system and then go to the locker to get that.
